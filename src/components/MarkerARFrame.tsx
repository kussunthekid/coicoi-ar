...
const MarkerARFrame = () => {
  ...
  const initializeAR = async () => {
    ...
    try {
      ...
      // カスタムUIとA-Frame sceneをHTMLに追加
      containerRef.current.innerHTML = customScanUI + sceneHTML;
      ...
      setTimeout(() => {
        ...
        if (closeButton) {
          closeButton.addEventListener('click', async () => {
            console.log('❌ Stopping AR from custom UI button');
            await stopAR();
          });
        }
      }, 100);
